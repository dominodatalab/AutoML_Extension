...
def create_app() -> FastAPI:
    ...
    async def capture_auth_header(request: Request, call_next):
        ...
        # NOTE: resolve_viewing_user() is disabled — the Domino sidecar
        # proxy does not support /api/users/v1/self (returns 500) and
        # the failing requests destabilize the proxy for other API calls.
        # RBAC owner filtering still works via the domino-username header
        # fallback in get_request_owner().
        try:
